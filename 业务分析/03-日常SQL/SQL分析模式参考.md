...
`dw_dwm.dwm_mg_log_abtest_user_d_1d` → AB实验真实命中用户日表（真实上报口径，不用ADS预分配表）
...
真实命中人群（`dwm_mg_log_abtest_user_d_1d`）∩ 公共/电商埋点行为（同天+同人+同平台）→ 天×实验组输出 启动/即享首页/商详(新老埋点拆分)/确认页/支付页/弹窗曝光/弹窗点击 UV + 弹窗点击率。

完整模板与口径说明见 [[技能库/AB实验分析SQL模板]]。
